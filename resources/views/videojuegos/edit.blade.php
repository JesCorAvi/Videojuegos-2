<x-app-layout>
    ...
    <form method="POST" action="{{ route('videojuegos.update', $videojuego) }}" class="max-w-sm mx-auto" method="POST" enctype="multipart/form-data">
        @csrf
        @method("PUT")
        <div class="mb-5">
            <label for="nombre" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombre del
                juego</label>
            <input value="{{$videojuego->titulo}}" type="nombre" name="titulo" id="nombre"
        ...
        <div class="mb-5">
            ...
            <input value="{{$videojuego->anyo}}" type="number" name="anyo" id="anyo"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                required />
        </div>
        <div class="mb-5">
            <label for="nombre" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Desarrolladora</label>
            <select name="desarrolladora_id">
                @foreach ($desarrolladoras as $desarrolladora)
                    <option value="{{$desarrolladora->id}}" {{$videojuego->desarrolladora->id == $desarrolladora->id ? "selected" : ""}}>{{$desarrolladora->nombre}}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-5">
